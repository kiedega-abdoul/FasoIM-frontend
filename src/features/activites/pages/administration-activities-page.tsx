...
export function AdministrationActivitiesPage() {
  ...
  const filteredSeances = useMemo(() => seances.filter((item) => item.type_seance === "ACTIVITE" && `${item.titre} ${item.centre.nom}`.toLowerCase().includes(search.toLowerCase())), [seances, search])
  ...
  async function loadFormateurs(currentSessionId: number | null, currentCentreId: number | null) {
    ...
    try {
      ...
      const rows = assignments
        .filter((item) => item.est_active && item.role_codes.includes("FORMATEUR"))
        .map((item) => ({
          id: item.acteur.id,
      ...
      setMessage(
        ...
        "Impossible de charger les formateurs de ce centre."
      )
      setFormateurs([])
      setFormateurId(null)
    }
  ...
  }, [assignment?.centre_id, isCenterManager])

  useEffect(() => {
    queueMicrotask(() => {
      void loadPlanning(sessionId, centreId)
    })
  }, [sessionId, centreId])
  ...
  async function program(event: FormEvent<HTMLFormElement>) {
    ...
    if (!formateurId) { setMessage("Choisissez le formateur chargé de la séance."); return }
    setSaving(true); setMessage("")
    const common = {
      ...
      module_activite_id: activityId,
      formateur_id: formateurId,
    }
    try {
      await activitesApi.createSeance({ ...common, type_seance: "ACTIVITE", statut: "PLANIFIEE" })
      form.reset()
      setShowProgram(false)
      await loadPlanning(sessionId, centreId)
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <section className="rounded-3xl bg-primary px-6 py-7 text-primary-foreground shadow-sm">
        ...
        <p className="mt-2 max-w-4xl text-base text-primary-foreground/85">{isCenterManager ? "Planifiez les séances de votre centre à partir des activités définies par l’administration, ou créez une séance autonome sans modifier le catalogue national." : "Les activités forment un catalogue national indépendant. Les séances sont ensuite programmées pour une session et un centre. Les séances programmées sont des activités avec suivi de présence."}</p>
      </section>
      ...
      {!isCenterManager && <div className="grid gap-4 md:grid-cols-2">
        ...
        <button onClick={() => setView("planning")} className={`rounded-2xl border p-5 text-left ${view === "planning" ? "border-primary bg-primary/5 ring-2 ring-primary/20" : "bg-background"}`}>
          <CalendarDays className="size-7 text-primary" /><h2 className="mt-3 text-xl font-bold">Planning des séances</h2><p className="mt-1 text-sm text-muted-foreground">Choisir une session, un centre et programmer une activité.</p>
        </button>
      </div>}
      ...
      {!isCenterManager && view === "catalogue" ? (
        ...
        <div className="space-y-6">
          ...
          {sessionId && centreId && <Card><CardContent className="space-y-5 p-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-bold">{isCenterManager ? "Programmer une séance" : "3. Programmer une séance"}</h2><p className="text-muted-foreground">{isCenterManager ? "Associez une activité du catalogue national ou choisissez « Sans activité du catalogue » pour une séance autonome." : "Associez une activité du catalogue national ou choisissez « Sans activité du catalogue » pour une séance autonome."}</p></div>{permissions.includes(P.PLAN_SEANCE) && <Button size="lg" className="h-12 px-6 text-base" onClick={() => setShowProgram(!showProgram)}><Plus className="mr-2 size-5" />Programmer une séance</Button>}</div>
            {showProgram && <form onSubmit={program} className="space-y-6 rounded-2xl border bg-muted/20 p-6">
              <div><p className="mb-3 font-medium">Activité associée — facultative</p><div className="grid max-h-64 gap-3 overflow-y-auto md:grid-cols-2 xl:grid-cols-4"><ChoiceCard selected={activityId === null} title="Sans activité du catalogue" onClick={() => setActivityId(null)} />{activities.map((item) => <ChoiceCard key={item.id} selected={activityId === item.id} title={item.titre} subtitle={item.code} onClick={() => setActivityId(item.id)} />)}</div></div>
              <div><p className="mb-3 font-medium">Formateur chargé de la séance — obligatoire</p><p className="mb-3 text-sm text-muted-foreground">Seuls les acteurs ayant le rôle Formateur dans cette session et ce centre sont proposés.</p><div className="grid max-h-64 gap-3 overflow-y-auto md:grid-cols-2 xl:grid-cols-4">{formateurs.map((item) => <ChoiceCard key={item.affectationId} selected={formateurId === item.id} title={item.nom} subtitle="Formateur" onClick={() => setFormateurId(item.id)} />)}</div></div>
              ...
              <Button type="submit" size="lg" className="h-12 min-w-64 text-base" disabled={saving}>{saving ? <LoaderCircle className="mr-2 size-5 animate-spin" /> : <CalendarDays className="mr-2 size-5" />}Programmer</Button>
            </form>}
