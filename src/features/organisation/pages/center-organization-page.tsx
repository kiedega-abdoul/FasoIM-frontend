  ...
  return <>
    ...
    {sessionId && centerId ? <form onSubmit={submit} className="space-y-6">
      ...
      <Card><CardContent className="grid gap-5 p-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Field label="Seuil de division en plusieurs sections *"><Input disabled={!canEdit} className="h-12" type="number" min={2} value={form.seuil_division_sections} onChange={(event) => setField("seuil_division_sections", Number(event.target.value))} required /><p className="mt-2 text-xs leading-5 text-muted-foreground">En dessous de ce seuil, une seule section est créée si sa capacité maximale suffit. À partir du seuil, l’effectif peut être réparti dans plusieurs sections.</p></Field>
        <Field label="Capacité maximale d’une section *"><Input disabled={!canEdit} className="h-12" type="number" min={1} value={form.capacite_max_section} onChange={(event) => setField("capacite_max_section", Number(event.target.value))} required /></Field>
        <Field label="Seuil de division en plusieurs groupes *"><Input disabled={!canEdit} className="h-12" type="number" min={2} value={form.seuil_division_groupes} onChange={(event) => setField("seuil_division_groupes", Number(event.target.value))} required /><p className="mt-2 text-xs leading-5 text-muted-foreground">Ce seuil est appliqué séparément dans chaque section. En dessous, un seul groupe est créé si sa capacité maximale suffit.</p></Field>
        <Field label="Capacité maximale d’un groupe *"><Input disabled={!canEdit} className="h-12" type="number" min={1} value={form.capacite_max_groupe} onChange={(event) => setField("capacite_max_groupe", Number(event.target.value))} required /></Field>
      ...
      <Card><CardContent className="grid gap-5 p-6 sm:grid-cols-2">
        ...
        <Field label="Consignes du centre"><textarea disabled={!canEdit} rows={4} className={textAreaClass} value={form.consignes_internes} onChange={(event) => setField("consignes_internes", event.target.value)} /></Field>
