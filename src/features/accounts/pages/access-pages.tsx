...
export function PermissionDetailPage(){const{permissionId}=useParams();const[p,setP]=useState<Permission|null>(null);useEffect(()=>{if(permissionId)accountsApi.permission(+permissionId).then(setP)},[permissionId]);if(!p)return<Loading/>;return <><PageHeader title={p.libelle} backTo="/app/permissions"/><Card><CardContent className="space-y-5 p-7"><p><b>Domaine :</b> {accessDomainLabel(p.module)}</p><p><b>Description :</b> {p.description||'—'}</p><p><b>Statut :</b> {p.statut}</p></CardContent></Card></>}
