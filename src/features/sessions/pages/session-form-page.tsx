...
export function SessionFormPage({ edit = false }: { edit?: boolean }) {
  ...
  return <><PageHeader title={edit ? "Modifier la session" : "Créer une session"} description="Renseignez uniquement les informations générales. L’organisation détaillée est définie séparément." backTo={edit ? `/app/sessions/${sessionId}` : "/app/sessions"} />{error && <div className="mb-5"><ErrorBox message={error} /></div>}<form onSubmit={submit} className="space-y-6"><Card><CardContent className="p-6 sm:p-8"><h2 className="text-xl font-semibold">Informations générales</h2><p className="mt-1 text-sm text-muted-foreground">Le code et le numéro de promotion sont calculés automatiquement.</p><div className="mt-6 grid gap-5 sm:grid-cols-2">
